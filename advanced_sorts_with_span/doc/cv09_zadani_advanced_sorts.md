...
Názvy proměnných v předpřipraveném kódu předpokládají použití názvů proměnných tak, jak byly uvedeny v přednáškách.
  ...
  - `vector_element_print_if()` - funkce pro tisk hodnoty na základě podmínky. Pokud má první parametr 
     ...
     Funkce je doplněna atributem `[[maybe_unused]]` definovaným v normě jazyka `C23`, který zajištuje potlačení varování při překladu v okamžiku, kdy jsou všechny ladicí výpisy ve všech třídicích funkcích zakomentovány a funkce `vector_element_print_if()` tedy není nikde volána.
...
`$Id: cv09_zadani_advanced_sorts.md 3319 2026-04-02 17:58:51Z petyovsky $`
