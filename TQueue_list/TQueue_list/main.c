 ...
 *  \author Petyovský
 *  \version 2026
 *  $Id: main.c 3215 2026-02-20 16:26:16Z petyovsky $
 */
...
#include "main_support.h"
  ...
  */
   ...
   *  \details Tisk hodnoty elementu fronty, na který ukazuje pøedaný iterátor.
   *  \param[in] aIter Ukazatel na existující iterátor
   */
void print_element(const struct TQueueIterator *aIter)
{
	printf("<" TQUEUE_ELEMENT_FRMSTR "> ", queue_iterator_value(aIter));
}

/** \brief Porovnání hodnoty elementu fronty pomocí iterátoru s hodnotou 43
 *  \details Pøíklad predikátové funkce. Porovnání hodnoty elementu fronty, na který ukazuje pøedaný iterátor s hodnotou 43.
 *  \param[in] aIter Ukazatel na existující iterátor
 *  \return \c true pokud je hodnota elementu fronty na který ukazuje iterátor rovna hodnotì 43
 */
[[nodiscard]] bool is_element_with_value_43(const struct TQueueIterator *aIter)
{
...
int main(int argc, char *argv[])
{
	...
	UNLESS(queue_push(&queue1, 0));
	printf("queue1 is empty: %s\n", bool_to_cstring(queue_is_empty(&queue1)));

	UNLESS(queue_push(&queue1, 222));
	...
	UNLESS(queue_push(&queue1, 42));
	UNLESS(queue_push(&queue1, 43));
	UNLESS(queue_push(&queue1, 40));
	...
	while(!queue_is_empty(&queue1))
		UNLESS(queue_pop(&queue1), goto cleanup_main);

	UNLESS(queue_push(&queue1, 99));
	queue_for_each(queue_iterator_begin(&queue1), print_element);
	putchar('\n');
