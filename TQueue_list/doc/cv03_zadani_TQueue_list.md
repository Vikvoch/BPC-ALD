...
Dokončete implementaci níže uvedených funkcí realizujících operace pro [ADT](https://cs.wikipedia.org/wiki/Abstraktn%C3%AD_datov%C3%BD_typ "Abstraktní datový typ (ADT) je v informatice výraz pro typy dat, které jsou nezávislé na vlastní implementaci. Hlavním cílem je zjednodušit a zpřehlednit program, který provádí operace s daným datovým typem.") fronta (Queue). V dodaném příkladu také implementujte operace pro mutable [iterátor](https://en.wikipedia.org/wiki/Iterator_pattern "Návrhový vzor iterátor představuje jednoduchý abstraktní datový typ, jehož účelem je obecná reprezentace pozice v kontejneru. Umožňuje zobecnit algoritmus procházení všemi elementy kontejneru tak, aby nebyl závislý na konkrétní implementaci kontejneru.") ADT fronta a následně při jeho použití ověřte správnou funkci tohoto rozhraní.
...
Pokuste se odpovědět si na tyto otázky:
 ...
 - Jak tento ADT funguje?
 - Pro jaký typ úloh je abstraktní datový typ fronta vhodný?
...
V následující kapitole je popsána stručně činnost - správné návratové hodnoty si již doplňte sami.
	...
	void queue_destroy(struct TQueue *aQueue);
		// Zrušení obsahu fronty. Odstraní (dealokuje) všechny uzly, které fronta obsahuje.
		// Pokud parametr aQueue není NULL,
		// zapamatuj si v dočasné proměnné odkaz na počáteční uzel seznamu,
...
Funkce `queue_for_each_n()` s prototypem: \code{.c}
...
\endcode   
Prvním parametrem funkce je iterátor `aIter`, jenž je předem asociovaný se zvolenou frontou a který tak definuje počáteční element pro zvolenou operaci.   
Druhým parametrem je hodnota `aN` udávající počet elementů fronty, na které se má zavolat funkce aOperation.   
Třetím parametrem funkce je ukazatel `aOperation` definující funkci (např. `print_element()`), jenž je volána na prvních `aN` elementů fronty, od elementu na který směřuje iterátor `aIter`, až do počtu `aN` elementů nebo po konec fronty.   
Funkce pomocí předaného iterátoru `aIter` prochází elementy fronty a pro každý aktuální element až do počtu `aN` zavolá funkci `aOperation`. Funkce tedy umožňuje **provést stejnou operaci**, kterou definuje parametr `aOperation`, se zadaným počtem elementů fronty od pozice určené iterátorem.   
Návratovou hodnotou funkce je hodnota iterátoru ukazujícího za `aN`-tý element fronty, nebo neplatný iterátor pokud fronta neměla dostatek elementů.   
Tato univerzální funkce je využitelná např. pro tisk prvních N elementů fronty.   
...
// Precondition: queue1: {41 42 43 43 44 45}
	...
	// Ukázka tisku prvních tří elementů fronty na samostatný řádek
	it = queue_for_each_n(queue_iterator_begin(&queue1), 3, print_element);
	putchar('\n');

	// Na další řádek dotiskneme zbylé elementy fronty, ovšem pouze pokud další elementy ve frontě existují
	if(queue_iterator_is_valid(&it))
		{
		queue_for_each(it, print_element);
		putchar('\n');
		}
...
---
`$Id: cv03_zadani_TQueue_list.md 3211 2026-02-20 16:10:14Z petyovsky $`
