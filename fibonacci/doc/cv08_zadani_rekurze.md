...
Cvičení je věnováno implementaci [rekurzivních funkcí](https://cs.wikipedia.org/wiki/Rekurze#Matematika). Programovací technika [rekurze](https://cs.wikipedia.org/wiki/Rekurze_(programov%C3%A1n%C3%AD)) bude procvičena na dvou úlohách:
...
Napište funkce realizující výpočet [Fibonacciho posloupnosti](https://cs.wikipedia.org/wiki/Fibonacciho_posloupnost) pomocí následujících metod:
 ...
 4. funkce `fibonacci4()` - implementace pomocí [koncového volání rekurze](https://cs.wikipedia.org/wiki/Koncov%C3%A1_rekurze).

### Soubory projektu `fibonacci`

Soubor `fibonacci.h` - obsahuje deklarace funkcí, které se mají realizovat. Navíc je zde, 
pro metodu se stromovou rekurzí a memoizací, deklarace proměnné memoizeru: `::m_fibonacci3` a [wrapper funkce](https://en.wikipedia.org/wiki/Wrapper_function): `fibonacci3_wrapper()`.
...
zavolá dodanou funkci (jejíž adresa je předána pomocí parametru `aFibFunc`) pro všechny vstupní parametry (zadané v poli `aTestSet`), porovná získané výsledky s výsledky vzorovými a také změří čas výpočtu. Funkce vrací hodnotu `true` pokud všechny výsledky vypočtené testovanou funkcí jsou shodné se vzorovými.
...
Soubor `memoizer.h` - obsahuje definice funkcí pro využití mechanizmů [memoizace](https://knihy.nic.cz/media/filer_public/39/19/3919a02c-cbb8-43b2-8abb-6362490ae0ab/pruvodce_labyrintem_algoritmu_v2.pdf) (viz str. 301).
...
#### Princip memoizace:
Memoizace (česky tabelace příp. memorování) - je optimalizační technika hojně využívaná ve [funkcionálním programování](https://cs.wikipedia.org/wiki/Funkcion%C3%A1ln%C3%AD_programov%C3%A1n%C3%AD), kterou poprvé popsal a využil [David Michie](https://en.wikipedia.org/wiki/Donald_Michie). Její princip spočívá v tom, že se jednou spočtená
výsledná hodnota časově náročné funkce zapamatuje (např. do tabulky) pro příští použití a to společně s požadovanými vstupními argumenty memoizované funkce.

Je-li při dalším volání požadován výsledek pro již zapamatovaný vstupní argument funkce, potom se jako hodnota funkce vrací zapamatovaná hodnota výsledku, aniž by se požadovaná časově náročná funkce volala. V ostatních případech se  výpočet hodnoty funkce musí provést a tento nový výsledek se opět archivuje pro příští využití.
...
Funkce:
  ...
  - `fibonacci3()` - 
     ...
     je odkaz na strukturu se zapamatovanými prvky (`::m_fibonacci3`) a požadovaná hodnota.
...
implementace funkce `wildcard_match()`. Projekt `unit_tests` má jediný soubor.
...
Další možné testovací řetězce (kromě těch, které jsou v modulu `unit_tests`):
...
---
`$Id: cv08_zadani_rekurze.md 3298 2026-03-27 11:36:37Z petyovsky $`
