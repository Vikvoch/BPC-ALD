...
int main(int argc, char *argv[])
{
	...
	fclose(fsrc);
#else
	bool initialized = vector_init_random(&vector_orig, 5/*0'000*/);
#endif
	...
	/// \bug Zvolte vhodnout tøídicí metodu, která splòuje podmínku stability. (\fileinfo{filename}:\lineinfo)
	select_sort(&vector_orig);

	vector_element_set_comparator(EByFirstName);
	/// \bug Zvolte správné poøadí stabilních a nestabilních metod tøídìní dle jednotlivých klíèù. (\fileinfo{filename}:\lineinfo)
	select_sort(&vector_orig);

	//vector_for_each(vector_iterator_begin(&vector_orig), print_element);
	vector_for_each_n(vector_iterator_begin(&vector_orig), 20, print_element);
	putchar('\n');
