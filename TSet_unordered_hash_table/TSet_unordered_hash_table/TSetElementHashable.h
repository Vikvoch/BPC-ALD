#ifndef TSET_ELEMENT_HASHABLE_H
#define TSET_ELEMENT_HASHABLE_H
/** \file TSetElementHashable.h
 *  \brief Definice typu Hashable Basic SetElement a implementace API
 *  \author Petyovský
 *  \version 2026
 *  $Id: TSetElementHashable.h 3361 2026-04-20 14:36:03Z petyovsky $
 */

#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
#include <stdint.h>
#include <assert.h>
#include "check.h"

 /** \defgroup TSetElementHashable 1. Hashable SetElement
  *  \brief Definice datového typu Hashable SetElement a jeho funkcí
  *  \{
  */

extern size_t gSetElementComparisons;	///< Deklarace globální promìnné realizující poèítadlo operací porovnání dvou hodnot typu SetElement

typedef int TSetElement;				///< Definice typu SetElement
#define TSET_ELEMENT_FRMSTR "d"			///< Definice formátovacího øetìzce (ovšem bez znaku `%`) urèeného pro souborové operace s typem SetElement

/** \brief Výpoèet hash hodnoty elementu
 *  \details Provede výpoèet hash hodnoty elementu pøedaného pomocí ukazatele.
 *  \param[in] aElement Ukazatel hodnotu elementu
 *  \return Hodnota typu \c size_t pøedstavující hash hodnotu elementu
 *  \attention Funkce ovìøuje platnost ukazatele \b pouze pøi pøekladu v režimu `Debug`, kdy pomocí `assert` hlásí bìhovou chybu!
 */
[[nodiscard]] static inline size_t set_element_hash_value(const TSetElement *aElement)
{
	assert(aElement);
	size_t hash_value = 0;
	uint8_t *hash_value_ptr = (uint8_t *)&hash_value;
	const uint8_t *element_ptr = (const uint8_t *)aElement;

	for(size_t i = 0, j = 0; i < sizeof(TSetElement); ++i)
	{
		hash_value_ptr[j] += element_ptr[i];
		if(++j == sizeof(size_t))
			j = 0;
	}
	return hash_value;
}

/** \brief Porovnání dvou elementù
 *  \details Provede trojcestné porovnání hodnot dvou elementù, pøedaných pomocí ukazatelù.
 *  \param[in] aLeft Ukazatel na levou porovnávanou hodnotu (tzv. LHS - Left Hand Side)
 *  \param[in] aRight Ukazatel na pravou porovnávanou hodnotu (tzv. RHS - Right Hand Side)
 *  \retval -1 Pokud (LHS < RHS)
 *  \retval  0 Pokud (LHS = RHS)
 *  \retval +1 Pokud (LHS > RHS)
 *  \attention Funkce ovìøuje platnost obou ukazatelù \b pouze pøi pøekladu v režimu `Debug`, kdy pomocí `assert` hlásí bìhovou chybu!
 */
[[nodiscard]] static inline int set_element_comparator(const TSetElement *aLeft, const TSetElement *aRight)
{
	assert(aLeft);
	assert(aRight);
	++gSetElementComparisons;
	if(*aLeft == *aRight)
		return 0;
	if(*aLeft < *aRight)
		return -1;
	return 1;
}

/** \brief Naètení elementu ze souboru
 *  \details Naète hodnotu elementu z pøedem otevøeného souboru.
 *  \param[in,out] aElement Ukazatel na místo v pamìti urèené pro naètení hodnoty
 *  \param[in,out] aInputFile Ukazatel na soubor otevøený v módu pro ètení
 *  \return \c true pokud byla hodnota elementu ze souboru úspìšnì naètena
 *  \attention Funkce ovìøuje platnost obou ukazatelù \b pouze pøi pøekladu v režimu `Debug`, kdy pomocí `assert` hlásí bìhovou chybu!
 */
[[nodiscard]] static inline bool set_element_load_file(TSetElement *aElement, FILE *aInputFile)
{
	assert(aElement);
	assert(aInputFile);
	return fscanf(aInputFile, "%" TSET_ELEMENT_FRMSTR, aElement) == 1;
}

/** \brief Uložení elementu do souboru
 *  \details Uloží hodnotu elementu do pøedem otevøeného souboru.
 *  \param[in] aElement Hodnota elementu urèená pro uložení do souboru
 *  \param[in,out] aOutputFile Ukazatel na soubor otevøený v módu pro zápis
 *  \return \c true pokud byla hodnota elementu do souboru úspìšnì uložena
 *  \attention Funkce ovìøuje platnost ukazatele \p aOutputFile \b pouze pøi pøekladu v režimu `Debug`, kdy pomocí `assert` hlásí bìhovou chybu!
 */
[[nodiscard]] static inline bool set_element_store_file(TSetElement aElement, FILE *aOutputFile)
{
	assert(aOutputFile);
	return fprintf(aOutputFile, "%" TSET_ELEMENT_FRMSTR " ", aElement) >= 0;
}

/** \brief Vrací nový element s náhodnou hodnotou
 *  \details Vytváøí a vrací nový element inicializovaný pomocí náhodné hodnoty.
 *  \return Nový element obsahující náhodnou hodnotu
 */
[[nodiscard]] static inline TSetElement set_element_random_value(void)
{
	return (TSetElement) { (TSetElement)rand() };
}

/** \} TSetElementHashable */

#endif /* TSET_ELEMENT_HASHABLE_H */
