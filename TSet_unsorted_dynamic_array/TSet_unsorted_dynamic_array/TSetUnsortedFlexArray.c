 ...
 *  \brief Implementace API pro typ SetUnsortedFlexArray (Flexibilní, tzv. otevøené, pole pro realizaci množiny pomocí netøídìného dynamicky alokovaného pole)
 *  \author Petyovský
 *  \version 2026
 *  $Id: TSetUnsortedFlexArray.c 3361 2026-04-20 14:36:03Z petyovsky $
 */
...
#include "check.h"
  ...
  *  \details Privátní typ SetUnsortedFlexArray (nedostupný mimo funkce z tohoto zdrojového souboru) reprezentuje typ pro tzv. flexibilní (otevøené) pole hodnot typu SetElement.
  */
struct TSetUnsortedFlexArray {
	size_t iCapacity;			///< Skuteènì naalokovaná kapacita prvkù vnitøního dynamického pole
	TSetElement iElement[];		///< Flexibilní (otevøené) pole hodnot typu SetElement (nedostupné mimo funkce ze souboru TSetUnsortedFlexArray.c)
};
