...
využívá `struct THeapIterator`. Jednotlivé varianty používají pro přesuny/úpravu datových členů pomocné funkce:
 `heap_alg_sift_down()` a `heap_alg_heapify()` s parametrem `struct THeap`, nebo `heap_iterator_alg_sift_down()` a `heap_iterator_alg_heapify()` s parametrem`struct THeapIterator`.
...
Struktura `TSpan` je navržena tak, aby umožňovala pohled na data vektoru jiným způsobem, aniž je bylo nutné alokovat v paměti 
jejich novou kopii.
...
---
`$Id: cv10_zadani_powerful_sorts.md 3333 2026-04-10 15:15:04Z petyovsky $`
