...
Ujasněte si rozdíl mezi knihovními funkcemi [`malloc()`](https://en.cppreference.com/w/c/memory/malloc) a [`calloc()`](https://en.cppreference.com/w/c/memory/calloc). Obě funkce dynamicky alokují paměť o velikosti bajtů. Funkce `calloc(počet, velikost)` si oba parametry (počet a velikost typu) vynásobí sama. [Některé implementace](https://web.archive.org/web/20251107193239/https://locklessinc.com/articles/calloc/) navíc kontrolují, z bezpečnostních důvodů, případné přetečení výsledku operace násobení. Od normy jazyka [C23](https://en.wikipedia.org/wiki/C23_(C_standard_revision)) je tato kontrola již povinně naimplementována v těle funkce `calloc()`. Tato funkce následně po alokaci (na rozdíl od funkce `malloc()`) zajistí efektivní nulování přidělené paměti s pomocí mechanismu jádra operačního systému. Z toho plyne, že `malloc()` použijeme tam, kde paměť (ihned) plníme svými daty. Funkci `calloc()` použijeme v okamžiku, kdy nemáme data k přiřazení do prvků pole, nebo chceme využít automatickou inicializaci prvků na nuly.

Seznamte se s pojmem [invariant](https://cs.wikipedia.org/wiki/Invariant_(informatika)). Invarianta ADT je podmínka (predikát), která musí být splněna v každém okamžiku existence ADT. Pokud bychom tuto invariantní podmínku porušili, například vlivem chybné implementace v těle `vector_init()`, došlo by k situaci, kdy daný ADT obsahuje po svém vzniku nekonzistentní data a není tedy např. plně funkční. Čím dříve tuto situaci v programu zjistíme, tím rychleji můžeme problém opravit, tento přístup k návrhu programu bývá označován jako [DbC - Design by Contract](https://cs.wikipedia.org/wiki/N%C3%A1vrh_podle_kontraktu). Platnost této invariantní podmínky proto budeme vždy na začátku a na konci každé implementované funkce ADT testovat. Jakou podmínku bude tvořit invarianta pro námi implementovaný ADT Vektor?

1. Prostudujte dodaný projekt a v něm dodané zdrojové kódy.
   - Projekt si překopírujte do své pracovní složky a otevřete ho ve VS pomocí souboru `TVector.slnx`.
   ...
   - Prohlédněte si soubor `main_support.h`, kde jsou umístěny pomocné funkce pro měření času a makro pro určení počtu elementů pole `ITEMS_OF()`a také pomocné makro pro zkrácené testování návratových hodnot funkcí `UNLESS()`, viz praktická ukázka zpracování [chybových stavů v jazyce C](https://www.geeksforgeeks.org/using-goto-for-exception-handling-in-c), nebo případně diskuze [zde](https://stackoverflow.com/questions/788903/valid-use-of-goto-for-error-management-in-c).
...
2. Lokální funkce `vector_invariant()` testuje invariantní podmínku (predikát) definovaný pro ADT.

        // tato funkce bude volána pouze při validitě vstupního parametru - test ukazatele na hodnotu NULL proto nebudeme provádět
        // v těle ověříme platnost navržené invariantní podmínky pomocí vzájemných vztahů členských proměnných struktury TVector, tj. počet prvků pole (iSize) a počáteční adresu pole (iValues)
        // při úspěchu funkce vrátíme hodnotu true, jinak false
3. Funkce `vector_init()` alokuje pole o dané délce s nulovými elementy a inicializuje složky struktury `TVector`, jejíž adresa je předána jako parametr. Předpokládáme, že tato funkce zajišťuje prvotní inicializaci proměnné vektor.
        ...
        // při úspěchu funkce vrátíme hodnotu true, jinak false
4. Funkce `vector_init_random()` vytvoří vektor o dané délce s náhodnými hodnotami - do proměnné, která je předána jako parametr.
 Od funkce `vector_init()` se liší vložením náhodných hodnot do elementů vektoru (použijte připravenou funkci `vector_element_random_value()` z `TVectorElement.h`).
        ...
        // ve struktuře nastavíme složky reprezentující počet prvků pole (iSize) a počáteční adresu pole (iValues)
        // při úspěchu funkce vrátíme hodnotu true, jinak false
5. Funkce `vector_init_file()` vytvoří vektor o dané délce s danými hodnotami - do proměnné, která je předána jako parametr. Druhým parametrem je proměnná typu `FILE*` představující otevřený textový soubor obsahující informaci o počtu elementů vektoru a následným výčtem jednotlivých hodnot vektoru. (K načtení hodnoty elementu použijte připravenou funkci `vector_element_load_file()` ze zdrojového souboru `TVectorElement.h`).
        ...
        // ve struktuře nastavíme složky reprezentující počet prvků pole (iSize) a počáteční adresu pole (iValues)
        // při úspěchu funkce vrátíme hodnotu true, jinak false
6. Funkce `vector_destroy()` uvolní získané zdroje (alokovanou paměť) a nastaví vnitřní složky proměnné `TVector` pro budoucí další použití.

        // test vstupních parametrů - má-li parametr typu ukazatel hodnotu NULL, ukončíme funkci
        // uvolnění paměti
        // vynulujeme složky struktury TVector
7. Funkce `vector_value()` vrátí hodnotu elementu na dané pozici vektoru.
...
8. Funkce `vector_size()` vrátí počet elementů vektoru.
...
9. Funkce `vector_resize()` změní počet prvků alokovaného pole na novou velikost a uchová hodnoty platných elementů. Předpokládá se, že vektor předaný jako parametr již byl korektně inicializován a může tedy obsahovat ukazatel na alokovanou paměť s platnými elementy. Tato paměť musí být dealokována, pokud již nebude nadále využívána.
        ...
        // pokud je nově požadovaný počet prvků vektoru nulový, dealokujeme paměť pole, vynulujeme jednotlivé složky a ukončím funkci s hodnotou true
        ...
        // při úspěchu funkce vrátíme hodnotu true, jinak false
10. Funkce `vector_clone()` vytvoří kopii elementů `aVector` (zdroj) do proměnné `aVectorClone`. Předpokládáme, že proměnná `aVectorClone` již mohla být využívána a může tedy obsahovat odkaz na alokovanou paměť s platnými elementy. Tato paměť musí být dealokována, pokud již nebude nadále využívána. Zvažte i situaci, kdy není třeba alokovat novou paměť, protože proměnná `aVectorClone` již má alokovánu paměť shodné velikosti jaká je pro tuto kopii potřeba.

        // test vstupních parametrů - má-li některý z parametrů typu ukazatel hodnotu NULL, ukončíme funkci
        // kontrola, zda originál a kopie jsou dvě různé proměnné typu TVector - pokud jsou stejné, funkci ukončíme
        // pokud není originál alokován - dealokujte i paměť kopie, vynulujte její složky a funkci ukončete
...
11. Funkce `vector_store_file()` představuje opak k načítání vektoru ze souboru. Data uloží ve stejném formátu, aby bylo možné jejich opětovně načtení pomocí funkce `vector_init_file()`. (K uložení hodnoty elementu do souboru použijte připravenou funkci `vector_element_store_file()` ze zdrojového souboru `TVectorElement.h`).
        ...
        // při úspěchu funkce vrátíme hodnotu true, jinak false
12. Funkce `vector_set_value()` má za úkol zapsat zadanou hodnotu na danou pozici v poli. Nejprve však musí provést kontrolu validity této operace.
...
---
`$Id: cv01_zadani_TVector.md 3194 2026-02-09 15:39:46Z petyovsky $`
