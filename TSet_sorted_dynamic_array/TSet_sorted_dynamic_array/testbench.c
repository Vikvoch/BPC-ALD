 ...
 *  \author Petyovský
 *  \version 2026
 *  $Id: testbench.c 3361 2026-04-20 14:36:03Z petyovsky $
 */
#include <stdio.h>
#include <time.h>
#include "testbench.h"
...
void print_element(const TTestedADTIterator *aIter)
{
	if(!set_element_store_file(set_iterator_value(aIter), stdout))
		fputs("Function 'set_element_store_file()' failed!\n", stderr);
}
...
/** \brief Provedení jednoho test z testovacího plánu na pøedaném ADT
 *  \details Funkce provede jeden test z testovacího plánu, zmìøí a vytiskne celkový výpoèetní èas a velikost pamìti, kterou funkce potøebovala k dokonèení výpoètu.
 *  \param[in,out] aADT Ukazatel na ADT na nìmž probìhne daný test
 *  \param[in] aTestPlan Ukazatel na test s urèenou: operací, polem vstupních hodnot a správných návratových hodnot
...
static bool test_run(TTestedADT *aADT, const struct TTestPlan *aTestPlan)
{
	...
	if(aTestPlan->iReportBefore)
	{
		...
		set_for_each(set_iterator_begin(aADT), print_element);
		putchar('\n');
	}
