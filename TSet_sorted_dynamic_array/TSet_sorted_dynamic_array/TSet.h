 ...
 *  \details Vkládá element s předanou hodnotou do množiny.
 *  \param[in,out] aSet Ukazatel na existující množinu
 *  \param[in] aValue Hodnota elementu vkládaná do množiny
 *  \return \c true pokud nebyl element s předanou hodnotou v množině nalezen a následně byl do množiny úspěšně vložen
 */
[[nodiscard]] bool set_insert(struct TSet *aSet, TSetElement aValue);

/** \brief Odstranění elementu z množiny
 *  \details Odstraní element s předanou hodnotou z množiny.
 *  \param[in,out] aSet Ukazatel na existující množinu
 *  \param[in] aValue Hodnota elementu odebíraného z množiny
 *  \return \c true pokud byl element s předanou hodnotou v množině nalezen a následně byl z množiny úspěšně odstraněn
 */
...
/** \brief Získání nového iterátoru ukazujícího na první element se zadanou hodnotou v množině, pokud byl nalezen
 ...
 *  \details Vytvoří a vrací nový iterátor, který je navázán (asociován) na zadanou množinu a ukazuje na její první element.
 *  \param[in] aSet Ukazatel na existující množinu
 ...
 *  \param[in] aIter Ukazatel na existující iterátor
 *  \param[in] aN Hodnota udávající počet prvních N elementů množiny, na které se má zavolat funkce \p aOperation
 *  \param[in] aOperation Ukazatel na funkci vracející \c void a mající jeden parametr typu ukazatel na iterátor
